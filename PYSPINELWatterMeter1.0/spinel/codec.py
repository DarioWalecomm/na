...
from spinel.hdlc import encode_wc
...
def transact(command_id, payload=bytes(), tid=SPINEL.HEADER_DEFAULT):
    pkt = encode_packet(command_id, payload, tid)
...
def ip_send_wc(pkt):
    pay = encode_i(SPINEL.PROP_STREAM_NET)
    ...
    transact(SPINEL.CMD_PROP_VALUE_SET, pay, SPINEL.HEADER_ASYNC)
...
def stream_tx(pkt):
    ...
    seriall = serial.Serial(port=dev, baudrate=baudrate, rtscts=rtscts)
