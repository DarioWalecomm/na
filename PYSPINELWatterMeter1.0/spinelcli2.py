...
from spinel.codec import ip_send_wc
...
class SpinelCliCmd(Cmd, SpinelCodec):
    ...
    def mqtt_subida(ip_to_mqtt,payoad_to_mqtt):
        str(payoad_to_mqtt)
    ...
    def on_mqtt_message(client, userdata, message):
        ...
        if A == "ID":
            ...
            ###
            ipnodo="2020:abcd::212:4b00:29b6:8dc3"
            ipborderrouter="2020:abcd::212:4b00:2949:5aef"
            src_addr = format(ipborderrouter)
            dest_addr = format(ipnodo)
            coap_req = ipv6_factory.build_coap_request(src_addr, dest_addr, ipv6.COAP_TYPE_CON, ipv6.COAP_METHOD_CODE_GET, OAD_REQ_URI_BASE + '/' + OAD_FWV_REQ_URI, tkl=DEFAULT_TKL, token=load_fwv_req_token) 
        ...
        #termina llega un mensaje en el topico instruccion
        #suscripcion mqttt
    
   
        #termina suscripcion

    # Funcion que inicia la conexion mqtt #
    def on_mqtt_connect(client, userdata, flags, rc):
        print("CONNECTED")
        print("Connected with result code: ", str(rc))
        client.subscribe("dtck-pub/gateway-1/f0965f63-44ea-4b87-a2fc-469c45841823/INSTRUCCION")
        # Termina Funcion que inicia la conexion mqt #
                    ...
                    if udp_pkt.header.dst_port == COAP_PORT:
                        ...
                        if coap_packet_type != "OAD_BLOCK_REQ":
                            print("\nCoAP packet received from {}: type: {} ({}), token: {}, code: {}.{:02d} ({}), msg_id: {}".format(
                                    ...
                                    #aqui se descompone el mensaje y se crea el topico al que se debe de enviar
                        ...
                        # LED GET response
                            ...
                            if len(p.payload) == 2:
                                ...
                                #mqtt_subida(pkt.ipv6_header.source_address,payload_to_mqtt_Send)
                                #la subida de los leds 
...
def main():
    ...
    print(stream_type)
    print(stream_descriptor)
    print(options.verbose)
    print(options.baudrate)
    print(options.rtscts)
...
def subscribing():
        client.on_message = SpinelCliCmd.on_mqtt_message
        client.loop_forever()    

# Codigo que solo se ejecuta una vez para iniciar la conexion hacer un hilo de prosesamiento y un loop infinito en ese hilo 
### MQTT ###
client = mqtt.Client()
client.on_connect = SpinelCliCmd.on_mqtt_connect
client.tls_set()
